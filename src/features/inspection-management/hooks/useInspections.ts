...
import type { Inspection } from "@/types/contract";
import { publishEvent, EVENT_TYPES } from "@infra/events";
import { showToast } from "@shared/ui/ToastContainer";

export function useInspections() {
  const [inspections, setInspections] = usePersistedState<Inspection[]>(
    "ics_inspections",
    initialInspections,
  );
  ...
  useEffect(() => {
    ...
    const newInspections = inspections.filter(
      (i) => !prevInspections.find((pi) => pi.id === i.id),
    );
    newInspections.forEach((inspection) => {
      publishEvent(
        EVENT_TYPES.INSPECTION_CREATED,
        {
      ...
      showToast(
        "success",
        "Inspection Created",
        `Inspection ${inspection.inspection_no || inspection.id} created`,
      );
    });

    const deletedInspections = prevInspections.filter(
      (i) => !inspections.find((ni) => ni.id === i.id),
    );
    deletedInspections.forEach((inspection) => {
      publishEvent(
        EVENT_TYPES.INSPECTION_DELETED,
        {
          inspectionId: inspection.id,
          inspectionNo: inspection.inspection_no,
        },
      ...
      showToast(
        "success",
        "Inspection Deleted",
        `Inspection ${inspection.inspection_no || inspection.id} removed`,
      );
    });
    ...
    updatedInspections.forEach((inspection) => {
      publishEvent(
        EVENT_TYPES.INSPECTION_UPDATED,
        {
          inspectionId: inspection.id,
          inspectionNo: inspection.inspection_no,
        },
      ...
      showToast(
        "success",
        "Inspection Updated",
        `Inspection ${inspection.inspection_no || inspection.id} updated`,
      );
    });
    ...
    return inspections.filter((inspection) => {
      ...
      const matchesSearch =
        ...
        (inspection.inspection_no &&
          inspection.inspection_no.toLowerCase().includes(query)) ||
        (inspection.inspector_name &&
          inspection.inspector_name.toLowerCase().includes(query));
