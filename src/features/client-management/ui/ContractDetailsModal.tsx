...
export function ContractDetailsModal({
  ...
  const canContractProgressWork = canAccessElement('client_contract_progress_work');
  const canContractProgressInvoice = canAccessElement('client_contract_progress_invoice');
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {canContractProgressWork && (
            <Card className={`p-4 rounded-xl border ${
              isDark ? 'border-slate-700/50 bg-slate-800/30' : 'border-slate-200/70 bg-white'
            }`}>
              ...
              <div className={`mt-2 h-1.5 rounded-full overflow-hidden ${isDark ? 'bg-slate-700/50' : 'bg-slate-200/70'}`}>
                <div
          ...
          {canContractProgressInvoice ? (
            ...
            <Card className={`p-4 rounded-xl border opacity-50 ${
              isDark ? 'border-slate-700/50 bg-slate-800/30' : 'border-slate-200/70 bg-white'
            }`}>
              <div className={`text-xs mb-1 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                Invoice Progress
              </div>
              <div className={`text-lg font-bold ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
                🔒 Locked
              </div>
        ...
        {canTariffsSection ? (
            ...
            {tariffs.length === 0 ? (
              ...
              <div className={`overflow-x-auto rounded-xl border ${
                ...
                <table className="w-full text-left text-xs">
                  <thead className={`${
                    isDark ? 'bg-slate-800/50 text-slate-400' : 'bg-slate-50/70 text-slate-500'
                  } text-[10px] uppercase tracking-wide`}>
                    <tr>
                      <th className="px-3 py-2 font-semibold">Description</th>
                      <th className="px-3 py-2 font-semibold">Unit</th>
                      <th className="px-3 py-2 font-semibold text-right">Rate</th>
                      <th className="px-3 py-2 font-semibold text-center">Total Performed Work</th>
                      {/* 🔗 client_tariffs_financial */}
                      {canTariffsFinancial && (
                        <>
                          <th className="px-3 py-2 font-semibold text-right">Total Value of Performed Works</th>
                          <th className="px-3 py-2 font-semibold text-right">Total Invoiced</th>
                        </>
                      )}
                    </tr>
                  </thead>
                  <tbody className={isDark ? 'divide-y divide-slate-700/50' : 'divide-y divide-slate-200/70'}>
                    {tariffs.map((tariff) => {
                      ...
                      return (
                        <tr key={tariff.id} className={isDark ? 'hover:bg-slate-800/30' : 'hover:bg-slate-50/50'}>
                          <td className={`px-3 py-2 font-medium ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>
                            {tariff.description}
                          </td>
                          <td className="px-3 py-2">
                            <Badge tone="indigo" className="text-[9px]">{tariff.unit.replace('_', ' ')}</Badge>
                          </td>
                          <td className="px-3 py-2 text-right font-mono">
                            {formatCurrency(tariff.rate, contract.currency)}
                          </td>
                          <td className="px-3 py-2 text-center font-mono">{consumed}</td>
                          {/* 🔗 client_tariffs_financial */}
                          {canTariffsFinancial && (
                            <>
                              <td className={`px-3 py-2 text-right font-mono font-bold ${isDark ? 'text-emerald-300' : 'text-emerald-700'}`}>
                                {formatCurrency(value, contract.currency)}
                              </td>
                              <td className={`px-3 py-2 text-right font-mono font-bold ${isDark ? 'text-indigo-300' : 'text-indigo-700'}`}>
                  ...
                  <tfoot className={isDark ? 'bg-slate-800/50 border-t-2 border-slate-600' : 'bg-slate-50/70 border-t-2 border-slate-300'}>
                    <tr>
                      <td colSpan={canTariffsFinancial ? 4 : 4} className={`px-3 py-2.5 text-sm font-bold uppercase tracking-wider ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>
                        💰 Total
                      </td>
                      {/* 🔗 client_tariffs_totals */}
                      {canTariffsTotals && (
                        <>
                          <td className={`px-3 py-2.5 text-right font-mono font-bold ${isDark ? 'text-emerald-300' : 'text-emerald-700'}`}>
                            {formatCurrency(
                              tariffs.reduce(
                          ...
                          <td className={`px-3 py-2.5 text-right font-mono font-bold ${isDark ? 'text-indigo-300' : 'text-indigo-700'}`}>
          ...
          <div className={`rounded-xl border-2 border-dashed p-8 text-center ${
            isDark ? 'border-slate-700/50 bg-slate-800/30' : 'border-slate-300/70 bg-slate-50/50'
          }`}>
