// src/views/clients/components/ClientDetails.tsx
...
export function ClientDetails({
  ...
  // 🔑 Computed values - قبل از conditional return
  ...
  if (!client) {
    return (
      <div className={`flex-1 flex items-center justify-center relative overflow-hidden min-h-[600px] ${
        isDark
          ?'bg-gradient-to-br from-slate-800 via-slate-900 to-indigo-950/30':'bg-gradient-to-br from-slate-50 via-white to-indigo-50/30'}`}>
      ...
      <div className={`border-b px-6 py-4 ${
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className={`text-xl font-bold ${isDark ?'text-slate-100':'text-slate-900'}`}>{client.name_en}</h3>
              <p className={`text-sm ${isDark ?'text-slate-300':'text-slate-600'}`} dir="rtl">{client.name_fa}</p>
            </div>
          </div>
            ...
            <div className={`rounded-lg border p-4 ${
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                <div>
                  ...
                  <div className={`font-mono text-xs ${isDark ?'text-slate-100':'text-slate-900'}`}>{client.national_id ||'—'}</div>
                </div>
                <div>
                  <div className={`text-[10px] uppercase font-semibold mb-1 ${isDark ?'text-slate-300':'text-slate-600'}`}>Registration No</div>
                  <div className={`font-mono text-xs ${isDark ?'text-slate-100':'text-slate-900'}`}>{(client as any).registration_no ||'—'}</div>
                </div>
                <div>
                  <div className={`text-[10px] uppercase font-semibold mb-1 ${isDark ?'text-slate-300':'text-slate-600'}`}>Economic Code</div>
                  <div className={`font-mono text-xs ${isDark ?'text-slate-100':'text-slate-900'}`}>{(client as any).economic_code ||'—'}</div>
                </div>
                <div>
                  <div className={`text-[10px] uppercase font-semibold mb-1 ${isDark ?'text-slate-300':'text-slate-600'}`}>Abbreviated Name</div>
                  <div className={`text-xs ${isDark ?'text-slate-100':'text-slate-900'}`}>{(client as any).abbreviated_name ||'—'}</div>
                </div>

                {/* Emails - با ref برای بستن با کلیک بیرون */}
                ...
                {/* Contact Persons - با ref برای بستن با کلیک بیرون */}
            ...
            <div className={`rounded-lg border p-4 ${
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                ...
                <div>
                  ...
                  <div className={`text-xs ${isDark ?'text-slate-100':'text-slate-900'}`}>{client.phone ||'—'}</div>
                </div>
				  ...
				  return (
					<div
					  ...
					  className={`rounded-lg border p-4 transition-colors cursor-pointer ${
						isDark ?'border-slate-700 hover:border-indigo-500':'border-slate-200 hover:border-indigo-300'} ${expiringInfo.expiring ? (isDark ?'border-amber-700/50 bg-amber-950/10':'border-amber-300 bg-amber-50/30') :''}`}
					>
