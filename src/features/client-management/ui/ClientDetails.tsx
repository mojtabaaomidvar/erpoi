...
export function ClientDetails({
  ...
  if (!client) {
    return (
      <div className={`flex-1 flex items-center justify-center relative overflow-hidden min-h-[600px] ${
        isDark ? 'bg-gradient-to-br from-slate-800 via-slate-900 to-indigo-950/30' : 'bg-gradient-to-br from-slate-50 via-white to-indigo-50/30'
      }`}>
  ...
  const handleContractClick = (contract: Contract) => {
    if (!canRead) return;
    if (!canAccessElement('client_contract_item')) return;
    onContractClick?.(contract);
  };

  // 🔗 Permission checks for each element
  const canEdit = canAccessElement('client_btn_edit');
  ...
  const canContractValue = canAccessElement('client_agreement_value');
  const canContractProgressWork = canAccessElement('client_agreement_progress_work');
  const canContractProgressInvoice = canAccessElement('client_agreement_progress_invoice');
    ...
    <div className={`col-span-1 lg:col-span-8 flex flex-col rounded-xl panel-3d overflow-hidden transition-all duration-300 ease-in-out ${
      ...
      <div className={`border-b px-6 py-4 ${
        ...
        <div className="flex items-center justify-between mb-4">
          <h2 className={`text-xs font-semibold uppercase tracking-wider ${isDark ? 'text-slate-200' : 'text-slate-600'}`}>
            Client Details
          </h2>
          <Button
                  ...
                  return (
                    <div
                      ...
                      className={`rounded-lg border p-4 transition-colors ${
                        canAccessElement('client_contract_item') ? 'cursor-pointer' : 'cursor-not-allowed opacity-70'
                      } ${
