...
export function useInvoices() {
  const [invoices, setInvoices] = usePersistedState<Invoice[]>(
    "ics_invoices",
    initialInvoices,
  );
  ...
  useEffect(() => {
    ...
    const newInvoices = invoices.filter(
      (i) => !prevInvoices.find((pi) => pi.id === i.id),
    );
    newInvoices.forEach((invoice) => {
      ...
      showToast(
        "success",
        "Invoice Created",
        `Invoice ${invoice.invoice_no || invoice.id} created`,
      );
    });

    const deletedInvoices = prevInvoices.filter(
      (i) => !invoices.find((ni) => ni.id === i.id),
    );
    deletedInvoices.forEach((invoice) => {
      ...
      showToast(
        "success",
        "Invoice Deleted",
        `Invoice ${invoice.invoice_no || invoice.id} removed`,
      );
    });
    ...
    updatedInvoices.forEach((invoice) => {
      ...
      showToast(
        "success",
        "Invoice Updated",
        `Invoice ${invoice.invoice_no || invoice.id} updated`,
      );
    });
