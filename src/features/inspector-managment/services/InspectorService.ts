// src/features/inspector-managment/services/InspectorService.ts
...
class InspectorService {
  ...
  async uploadResume(
    ...
    // 🔧 FIX: برگرداندن نام سفارشی به عنوان نام نمایشی
    return {
  ...
  async getAvailableUsersForIcsMember(currentEditingUserId?: string | null) {
    ...
    const usedUserIds = new Set(
      existingInspectors?.map((i) => i.user_id).filter(Boolean) || [],
    );

    const availableUsers = (allUsers || []).filter((user: any) => {
      if (user.id === currentEditingUserId) return true;
