...
import type { Inspector } from "@/types/contract";
import { publishEvent, EVENT_TYPES } from "@infra/events";
import { showToast } from "@shared/ui/ToastContainer";

export function useInspectors() {
  const [inspectors, setInspectors] = usePersistedState<Inspector[]>(
    "ics_inspectors",
    initialInspectors,
  );
  const [searchQuery, setSearchQuery] = useState("");
  const [filter, setFilter] = useState<
    "ALL" | "AVAILABLE" | "BUSY" | "ON_LEAVE"
  >("ALL");
  ...
  useEffect(() => {
    ...
    const newInspectors = inspectors.filter(
      (i) => !prevInspectors.find((pi) => pi.id === i.id),
    );
    newInspectors.forEach((inspector) => {
      publishEvent(
        EVENT_TYPES.INSPECTOR_CREATED,
        {
      ...
      showToast(
        "success",
        "Inspector Added",
        `${inspector.name_en || inspector.id} has been added`,
      );
    });

    const deletedInspectors = prevInspectors.filter(
      (i) => !inspectors.find((ni) => ni.id === i.id),
    );
    deletedInspectors.forEach((inspector) => {
      publishEvent(
        EVENT_TYPES.INSPECTOR_DELETED,
        {
          inspectorId: inspector.id,
          inspectorName: inspector.name_en,
        },
      ...
      showToast(
        "success",
        "Inspector Deleted",
        `${inspector.name_en || inspector.id} has been removed`,
      );
    });
    ...
    updatedInspectors.forEach((inspector) => {
      ...
      showToast(
        "success",
        "Inspector Updated",
        `${inspector.name_en || inspector.id} has been updated`,
      );
    });
  ...
  const filteredInspectors = useMemo(() => {
    return inspectors.filter((inspector) => {
      ...
      const matchesSearch =
        !query ||
        (inspector.name_en &&
          inspector.name_en.toLowerCase().includes(query)) ||
