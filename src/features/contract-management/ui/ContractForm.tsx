// src/features/contract-management/ui/ContractForm.tsx
// 🔐 RBAC: دسترسی‌ها بر اساس نقش کاربر
...
interface ContractFormProps {
  ...
  // 🔐 RBAC: اضافه شدن permission props
  canCreate?: boolean;
  canUpdate?: boolean;
}
...
export function ContractForm({
  ...
  // 🔐 RBAC: محاسبه permission فعلی بر اساس mode
  ...
  useEffect(() => {
    if (isOpen && !hasPermission) {
      showToast(
        "error",
        "Access Denied",
        `You do not have permission to ${mode === "add" ? "create" : "edit"} contracts`,
      );
      onClose();
    }
  ...
  const [formData, setFormData] = useState({
    ...
    source_email_date:
      initialData?.source_email_date || new Date().toISOString().split("T")[0],
    ...
    type: (typeFilter === "WORK_ORDER"
      ? "WORK_ORDER"
      : initialData?.type || "CONTRACT") as "CONTRACT" | "WORK_ORDER",
    contract_count: initialData?.contract_count || 1,
    description: initialData?.description || "",
    tariffs:
      initialData?.tariffLines ||
      ([
        {
          id: "1",
          description: "",
  ...
  const validateForm = () => {
    ...
    if (formData.type === "WORK_ORDER") {
      if (formData.source_type === "LETTER") {
        ...
        if (!formData.source_letter_image)
      ...
      if (!formData.tariffs || formData.tariffs.length === 0) {
        ...
        const emptyTariff = formData.tariffs.find(
          (t) =>
            !t.description.trim() ||
            !t.rate ||
            parseNumberInput(t.rate as string) <= 0,
        );
        if (emptyTariff)
      ...
      if (!formData.contract_count || formData.contract_count <= 0)
        newErrors.contract_count = "Contract count is required";
    }
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={mode === "add" ? "Add New Agreement" : "Edit Contract"}
      size="lg"
    >
      <div className="space-y-3">
        ...
        <div>
          <label className="mb-1.5 block text-xs font-semibold text-primary">
            ...
            {typeFilter !== "ALL" && (
              ...
              >
                🔒 Locked to{" "}
                {typeFilter === "CONTRACT" ? "Contract" : "Work Order"}
              </span>
            )}
          </label>
        ...
        {formData.type === "CONTRACT" ? (
          <>
            ...
            >
              <div className="grid grid-cols-4 gap-4">
                ...
                {/* 🔐 RBAC: فیلدهای مالی فقط با دسترسی */}
                {canViewFinancial ? (
                  <>
                    <div>
                      <label className="mb-1.5 block text-xs font-semibold text-primary">
                  ...
                  <>
                    <div className="col-span-2">
                      ...
                      <div className="w-full rounded-lg border-2 border-dashed border-slate-400 bg-slate-100 dark:bg-slate-800/30 py-2.5 px-3 text-sm text-slate-500 italic text-center">
            ...
            {/* 🔐 RBAC: ردیف‌های تعرفه فقط با دسترسی مالی */}
            {canViewFinancial ? (
              <TariffEditor
              ...
              >
                ...
                <h4
                  className={`text-sm font-semibold mb-2 ${isDark ? "text-slate-200" : "text-slate-700"}`}
                >
                  Tariff Editor Locked
                </h4>
                <p
                  className={`text-xs ${isDark ? "text-slate-400" : "text-slate-600"}`}
                >
                  You need financial access permission to edit tariffs.
                </p>
              </div>
            )}
            ...
            {/* 🔐 RBAC: Financial & Legal Terms فقط با دسترسی مالی */}
            {canViewFinancial ? (
              <div
                className={`rounded-lg border p-4 ${isDark ? "border-slate-700 bg-slate-800/30" : "border-slate-200 bg-slate-50/50"}`}
              >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                              ...
                              <input
                                ...
                                value={
                                  formData.adjustment.percentage
                                    ? formatNumberInput(
                                        String(formData.adjustment.percentage),
                                      )
                                    : ""
                                }
                                onChange={(e) =>
                    ...
                    >
                      ...
                      {formData.contract_modification.percentage ||
                        ...
                        (0 > 0 && (
                          <div
                            className={`mt-3 rounded-lg border p-3 ${isDark ? "border-indigo-700 bg-indigo-950/30" : "border-indigo-200 bg-indigo-50/50"}`}
                          >
                            <div className="grid grid-cols-2 gap-2 text-xs">
                              <div
                                className={`flex justify-between p-2 rounded ${isDark ? "bg-slate-800/50" : "bg-white/70"}`}
                              >
                                <span
                                  className={
                                ...
                                <span className="font-bold font-mono text-indigo-500">
                                  {formData.total_value > 0
                                    ? `+${formatCurrency((formData.total_value * (formData.contract_modification.percentage || 0)) / 100, formData.currency)}`
                                    : "—"}
                                </span>
                              </div>
                              <div
                              ...
                              >
                                ...
                                <span className="font-bold font-mono text-indigo-500">
                                  {formData.start_date && formData.end_date
                                    ? (() => {
                                        try {
                                          const [startJy, startJm, startJd] =
                                            formData.start_date
                                              .split("/")
                                              .map(Number);
                                          const [endJy, endJm, endJd] =
                                            formData.end_date
                                              .split("/")
                                              .map(Number);
                                          if (!startJy || !endJy) return "—";
                                          const startG = jalaali.toGregorian(
                                            startJy,
                                            startJm,
                                            startJd,
                                          );
                                          const endG = jalaali.toGregorian(
                                            endJy,
                                            endJm,
                                            endJd,
                                          );
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          setFormData({
                            ...
                            guarantee: {
                              ...formData.guarantee,
                              has_guarantee: !formData.guarantee.has_guarantee,
                            },
                          });
                        }}
                        ...
                        <input
                          ...
                          value={
                            formData.good_performance_percentage
                              ? formatNumberInput(
                                  String(formData.good_performance_percentage),
                                )
                              : ""
                          }
                          onChange={(e) =>
                      ...
                      <div className="relative">
                        <input
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              insurance_deduction_percentage: parseNumberInput(
                                e.target.value,
                              ),
                            })
                          }
              ...
              >
                ...
                <h4
                  className={`text-sm font-semibold mb-2 ${isDark ? "text-slate-200" : "text-slate-700"}`}
                >
                  Financial & Legal Terms Locked
                </h4>
                <p
                  className={`text-xs ${isDark ? "text-slate-400" : "text-slate-600"}`}
                >
                  You need financial access permission to edit financial terms.
                </p>
              </div>
            )}
          ...
          <>
            {/* Work Order Form - همون قبلی بدون تغییر */}
            <div>
              <label className="mb-1.5 block text-xs font-semibold text-primary">
                Source Type *
              </label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, source_ref: e.target.value })
                      }
                  ...
                  <div>
                    ...
                    <JalaaliDatePicker
                      ...
                      onChange={(date) =>
                        setFormData({ ...formData, source_letter_date: date })
                      }
                    ...
                    <label
                      ...
                      className={`flex items-center justify-between gap-2 w-full rounded-lg border-2 px-3 py-2.5 text-sm cursor-pointer transition-colors ${
                        formData.source_letter_image
                          ...
                          : isDark
                            ? "border-dashed border-slate-600 bg-muted text-secondary hover:border-indigo-500 hover:bg-slate-800"
                            : "border-dashed border-slate-300 bg-muted text-slate-600 hover:border-indigo-400 hover:bg-indigo-50"
                      }`}
                    >
                      {formData.source_letter_image ? (
                        <>
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            {formData.source_letter_image_preview && (
                              <img
                                src={formData.source_letter_image_preview}
                                alt="Preview"
                        ...
                        <div className="flex items-center gap-2">
                          <span>📎</span>
                          <span>
                            Click to attach letter image (JPG, PNG, PDF)
                          </span>
                        </div>
                      )}
                    </label>
            ...
            {/* 🔐 RBAC: Tariff Editor فقط با دسترسی مالی */}
            {canViewFinancial ? (
              <TariffEditor
                tariffs={formData.tariffs}
                onChange={(tariffs) =>
        ...
        >
          ...
          <Button onClick={handleSave}>
            💾 Save {formData.type === "CONTRACT" ? "Contract" : "Work Order"}
          </Button>
        </div>
      </div>
    </Modal>
