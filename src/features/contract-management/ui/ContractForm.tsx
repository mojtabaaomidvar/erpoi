      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <div className="flex gap-2">
            <button
              type="button"onClick={() => handleTypeChange('CONTRACT')}
              disabled={typeFilter !=='ALL'}
              className={`flex-1 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${
                formData.type ==='CONTRACT'?'bg-indigo-600 text-white shadow-md': (isDark ?'bg-muted text-secondary hover:bg-slate-700':'bg-slate-100 text-slate-600 hover:bg-slate-200')
              } ${typeFilter !=='ALL'&& formData.type !=='CONTRACT'?'opacity-40 cursor-not-allowed':''}`}
            >
              📄 Contract
            </button>
            <button
              type="button"onClick={() => handleTypeChange('WORK_ORDER')}
              disabled={typeFilter !=='ALL'}
              className={`flex-1 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${
                formData.type ==='WORK_ORDER'?'bg-amber-600 text-white shadow-md': (isDark ?'bg-muted text-secondary hover:bg-slate-700':'bg-slate-100 text-slate-600 hover:bg-slate-200')
          ...
          <>
            ...
            <div className={`rounded-lg border p-4 ${isDark ?'border-slate-700 bg-slate-800/30':'border-slate-200 bg-slate-50/50'}`}>
              <div className="grid grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="text"inputMode="numeric"value={formData.total_value ? formatNumberInput(String(formData.total_value)) :''}
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    {formData.adjustment.enabled && (
                      <div className="space-y-3">
                        <div className={`rounded-lg border p-3 ${isDark ?'border-slate-700 bg-slate-800/30':'border-slate-200 bg-slate-50/30'}`}>
                          ...
                          <div className="flex gap-2">
                            <button
                              type="button"onClick={() => setFormData({ ...formData, adjustment: { ...formData.adjustment, mode:'FIXED'} })}
                              className={`flex-1 px-3 py-2 rounded-lg text-xs font-semibold transition-all ${
                                formData.adjustment.mode ==='FIXED'?'bg-indigo-600 text-white shadow-md': (isDark ?'bg-slate-700 text-slate-300 hover:bg-slate-600':'bg-slate-200 text-slate-600 hover:bg-slate-300')
                              }`}
                            >
                              ✅ Fixed
                            </button>
                            <button
                              type="button"onClick={() => setFormData({ ...formData, adjustment: { ...formData.adjustment, mode:'TBD', percentage: 0 } })}
                              className={`flex-1 px-3 py-2 rounded-lg text-xs font-semibold transition-all ${
                                formData.adjustment.mode ==='TBD'?'bg-amber-600 text-white shadow-md': (isDark ?'bg-slate-700 text-slate-300 hover:bg-slate-600':'bg-slate-200 text-slate-600 hover:bg-slate-300')
                              }`}
                            >
                              ⏳ TBD
                            </button>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            ...
                            <input
                              type="text"inputMode="decimal"value={formData.adjustment.percentage ? formatNumberInput(String(formData.adjustment.percentage)) :''}
                  ...
                  <div className={`rounded-lg border p-4 ${
                    ...
                    <div className="relative">
                      <input
                        type="text"inputMode="decimal"value={formData.contract_modification?.percentage ? formatNumberInput(String(formData.contract_modification.percentage)) :''}
                        onChange={(e) => {
                        ...
                        className="w-full rounded-lg border px-3 py-2 pr-8 text-sm font-mono text-right input-themed"placeholder="0"/>
                ...
                <div className={`rounded-lg border p-4 ${
                  ...
                  <div className="flex items-center justify-between mb-3">
					...
					<button
					  ...
					  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
						formData.total_value <= 0
						  ?"opacity-40 cursor-not-allowed bg-slate-400": formData.guarantee.has_guarantee 
							?"bg-emerald-600": (isDark ?"bg-slate-700":"bg-slate-300")
					  }`}
					  title={
						formData.total_value <= 0 
						  ?"Enter Total Value first to enable guarantee":"Toggle performance guarantee"}
					>
					  <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full rounded-lg border px-3 py-2 text-sm font-mono text-right input-themed"placeholder="0.00"/>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className={`rounded-lg border p-4 ${isDark ?'border-slate-700 bg-slate-800/20':'border-slate-200 bg-slate-50/20'}`}>
                    ...
                    <div className="relative">
                      <input
                        type="text"inputMode="decimal"value={formData.good_performance_percentage ? formatNumberInput(String(formData.good_performance_percentage)) :''}
                        onChange={(e) => setFormData({ ...formData, good_performance_percentage: parseNumberInput(e.target.value) })}
                        className="w-full rounded-lg border px-3 py-2 pr-8 text-sm font-mono text-right input-themed"placeholder="10.00"/>
                      <span className={`absolute right-3 top-1/2 -translate-y-1/2 text-xs font-semibold ${isDark ?'text-slate-400':'text-slate-500'}`}>%</span>
                    </div>
                  </div>
                  ...
                  <div className={`rounded-lg border p-4 ${isDark ?'border-slate-700 bg-slate-800/20':'border-slate-200 bg-slate-50/20'}`}>
                    ...
                    <div className="relative">
                      <input
                        type="text"inputMode="decimal"value={formData.insurance_deduction_percentage ? formatNumberInput(String(formData.insurance_deduction_percentage)) :''}
                        onChange={(e) => setFormData({ ...formData, insurance_deduction_percentage: parseNumberInput(e.target.value) })}
                        className="w-full rounded-lg border px-3 py-2 pr-8 text-sm font-mono text-right input-themed"placeholder="16.67"/>
                      <span className={`absolute right-3 top-1/2 -translate-y-1/2 text-xs font-semibold ${isDark ?'text-slate-400':'text-slate-500'}`}>%</span>
                    </div>
                  </div>
                </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full rounded-lg px-3 py-2 text-sm input-themed"placeholder="Optional description..."/>
            </div>
          </>
        ) : (
          <>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <button
                  type="button"onClick={() => setFormData({ ...formData, source_type:'LETTER'})}
                  className={`flex-1 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${
                    formData.source_type ==='LETTER'?'bg-emerald-600 text-white shadow-md': (isDark ?'bg-muted text-secondary hover:bg-slate-700':'bg-slate-100 text-slate-600 hover:bg-slate-200')
                  }`}
                >
                  📄 Letter
                </button>
                <button
                  type="button"onClick={() => setFormData({ ...formData, source_type:'EMAIL'})}
                  className={`flex-1 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${
                    formData.source_type ==='EMAIL'?'bg-blue-600 text-white shadow-md': (isDark ?'bg-muted text-secondary hover:bg-slate-700':'bg-slate-100 text-slate-600 hover:bg-slate-200')
                  }`}
                >
                  📧 Email
                </button>
            ...
            {formData.source_type ==='LETTER'? (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full rounded-lg bg-card px-3 py-2.5 text-sm font-mono input-themed"placeholder="e.g., 1404/1234"/>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      type="file"id="letter-image-input"accept="image/*"onChange={(e) => {
                      ...
                      className="hidden"/>
                    <label
                      htmlFor="letter-image-input"className={`flex items-center justify-between gap-2 w-full rounded-lg border-2 px-3 py-2.5 text-sm cursor-pointer transition-colors ${
                        ...
                        <>
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            ...
                            <button
                              ...
                              className="p-1 text-rose-500 hover:text-rose-700 hover:bg-rose-100 rounded transition-colors"title="Remove image">
                              ✕
                            </button>
                          </div>
                        </>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="email"value={formData.source_email_from ||''}
                  ...
                  <div>
                    ...
                    <input
                      type="date"value={formData.source_email_date ||''}
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      type="file"id="email-file-input"accept=".msg,.eml"onChange={(e) => {
                        const file = e.target.files?.[0];
                        if (file) {
                          setFormData({
                            ...formData,
                      ...
                      className="hidden"/>
                    <label
                      htmlFor="email-file-input"className={`flex items-center justify-between gap-2 w-full rounded-lg border-2 px-3 py-2.5 text-sm cursor-pointer transition-colors ${
                      ...
                      {formData.source_file ? (
                        <>
                          <button
                            ...
                            className="flex items-center gap-2 flex-1 text-left hover:underline min-w-0"title="Click to open email">
                            <span>📧</span>
                          ...
                          <button
                            ...
                            className="shrink-0 p-1 text-rose-500 hover:text-rose-700 hover:bg-rose-100 rounded transition-colors"title="Remove file">
                            ✕
                          </button>
                        </>
                      ) : (
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                rows={3}
                className="w-full rounded-lg px-3 py-2 text-sm input-themed"placeholder="Additional details..."/>
            </div>
          </>
        )}
