...
export function ContractDetails({
  ...
  const { canAccessElement } = usePermissionMapping();  // 🔐 NEW

  const canViewFinancial = canAny(['contract:read', 'contract:view_all', 'contract:view_own']);

  // 🔐 RBAC: چک کردن دسترسی‌های UI Element
  const canBtnEdit = canUpdate && canAccessElement('contract_btn_edit');
  const canBtnApprove = canApprove && canAccessElement('contract_btn_approve');
  ...
  if (!contract) {
    return (
      <div className={`flex-1 flex items-center justify-center relative overflow-hidden min-h-[600px] ${
        isDark ? 'bg-gradient-to-br from-slate-800 via-card to-indigo-950/30' : 'bg-gradient-to-br from-slate-50 via-white to-indigo-50/30'
      }`}>
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 h-full overflow-hidden">
      ...
      <div className={`border-b border-theme px-6 py-4 ${
        isDark ? 'bg-gradient-to-r from-slate-800 to-card' : 'bg-gradient-to-r from-slate-50 to-white'
      }`}>
          ...
          {canBtnEdit && (
            <Button
              ...
              className={`gap-2 shadow-sm ${
                (contract.status === 'COMPLETED' || financialStatus === 'completed') ? 'opacity-50 cursor-not-allowed' : ''
              }`}
              title={
          ...
          {canInfoSection && (
            <div className="rounded-lg border border-theme p-4 bg-muted/30">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 lg:gap-4 text-sm">
                ...
                {/* 🔐 Financial Info - نیاز به canViewFinancial + permission مربوطه */}
                {canViewFinancial && (
                  <>
                ...
                {/* 🔒 Placeholder اگه canViewFinancial نداره */}
                {!canViewFinancial && (
