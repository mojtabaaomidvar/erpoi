  ...
  if (!contract) {
    return (
      <div className={`flex-1 flex items-center justify-center relative overflow-hidden min-h-[600px] ${
        isDark
          ?'bg-gradient-to-br from-slate-800 via-card to-indigo-950/30':'bg-gradient-to-br from-slate-50 via-white to-indigo-50/30'}`}>
  ...
  return (
	<div className="flex-1 flex flex-col min-h-0 h-full overflow-hidden">
      ...
      <div className={`border-b border-theme px-6 py-4 ${
        isDark
          ?'bg-gradient-to-r from-slate-800 to-card':'bg-gradient-to-r from-slate-50 to-white'}`}>
                ...
                <>
                  ...
                  <button
                    ...
                    className={`w-full rounded-lg px-3 py-2 text-xs font-semibold transition-all ${
                      userRole ==='admin'?'bg-amber-500 text-white hover:bg-amber-600 shadow-sm cursor-pointer': (isDark ?'bg-slate-700 text-slate-400 cursor-not-allowed opacity-60':'bg-slate-100 text-slate-400 cursor-not-allowed opacity-60')
                    }`}
