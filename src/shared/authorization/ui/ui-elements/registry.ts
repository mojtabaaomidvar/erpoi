...
class UIElementRegistry {
  ...
  register(module: string, elements: UIElement[]): void {
    ...
    this.modules.set(singularModule, {
      module: singularModule,
  ...
  /**
   * 🔍 گرفتن elements حساس (Sensitive)
   */
  getSensitiveElements(): UIElement[] {
    return this.getAllElements().filter((el) => el.sensitive);
  }
