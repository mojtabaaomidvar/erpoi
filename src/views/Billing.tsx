...
export function Billing() {
  ...
  const outstanding = invoices.filter((i) => i.status === "ISSUED").reduce((s, i) => s + i.total_amount, 0);
  const overdue = invoices.filter((i) => i.status === "OVERDUE").reduce((s, i) => s + i.total_amount, 0);
  const draft = invoices.filter((i) => i.status === "DRAFT").reduce((s, i) => s + i.total_amount, 0);
