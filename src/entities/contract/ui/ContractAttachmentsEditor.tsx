...
export function ContractAttachmentsEditor({
  ...
  const [activeCategory, setActiveCategory] = useState<
    "CONTRACT" | "ADDENDUM" | "CORRESPONDENCE" | "TECHNICAL"
  >("CONTRACT");
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!files) return;
    const newAttachments: ContractAttachment[] = Array.from(files).map(
      (file) => ({
  ...
  const counts = {
    ...
    CORRESPONDENCE: attachments.filter((a) => a.category === "CORRESPONDENCE")
      .length,
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex gap-1 rounded-lg border p-1 mb-3 text-xs ${
        ...
        >
          ...
          <span className="font-medium">
            ...
            <span className="text-indigo-500">
              {
                ATTACHMENT_CATEGORIES.find((c) => c.value === activeCategory)
                  ?.label
              }
            </span>
          </span>
      ...
      {filteredAttachments.length === 0 ? (
        <div
          className={`text-center py-6 text-xs ${isDark ? "text-slate-500" : "text-slate-400"}`}
        >
            ...
            >
              <span className="text-base">
                {att.name.toLowerCase().endsWith(".pdf")
                  ? "📄"
                  : att.name.toLowerCase().match(/\.(jpg|jpeg|png)$/)
                    ? "🖼️"
              ...
              <div className="flex-1 min-w-0">
                <div
                  className={`font-medium truncate ${isDark ? "text-slate-200" : "text-slate-800"}`}
                >
                  {att.name}
                </div>
                <div
                ...
                >
                  {att.file_object
                    ? `${(att.file_object.size / 1024).toFixed(1)} KB`
                    : ""}
                </div>
              </div>
              <button
