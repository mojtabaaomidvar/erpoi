...
export function TariffEditor({
  ...
  const updateTariff = (id: string, field: keyof TariffLine, value: any) => {
    const updated = tariffs.map((t) => {
      ...
      // محاسبه خودکار Total بر اساس Rate فرمت شده
      if (field === "rate") {
  ...
  const totalsByCurrency = useMemo(() => {
    ...
    tariffs.forEach((t) => {
      ...
      totals[curr] += t.total || parseNumberInput(t.rate);
    });
    return totals;
  }, [tariffs]);
  ...
  return (
    <div>
            ...
            <div className="grid grid-cols-12 gap-2">
              ...
              <div className="col-span-2">
                <select
                  value={tariff.unit}
                  onChange={(e) =>
                    updateTariff(tariff.id, "unit", e.target.value)
                  }
                  className="w-full rounded border px-2 py-1.5 text-xs input-themed"
                >
                  {["MAN_DAY", "DOCUMENT", "VESSEL", "LUMP_SUM"].map((u) => (
              ...
              <div className="col-span-1">
                <select
                  ...
                  onChange={(e) =>
                    updateTariff(tariff.id, "currency", e.target.value)
                  }
      ...
      {/* 🔧 FIX: استفاده از totalsByCurrency فقط اگر تعریف شده باشد */}
    </div>
  );
}
