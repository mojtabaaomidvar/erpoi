  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          label="Active Inspections"value={String(activeInspections)}
          delta="↑ 4 this week"tone="indigo"icon={<FileCheck className="h-5 w-5"/>}
        />
        <StatCard
          label="Revenue (30d)"value={formatCurrency(monthRevenue,"USD")}
          delta="↑ 12.4% vs last month"tone="emerald"icon={<Wallet className="h-5 w-5"/>}
        />
        <StatCard
          label="Open NCRs"value={String(openNcrs)}
          delta="↓ 1 from yesterday"tone="rose"icon={<AlertTriangle className="h-5 w-5"/>}
        />
        <StatCard
          ...
          tone="amber"icon={<TrendingUp className="h-5 w-5"/>}
        />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader
            title="Inspections & Revenue Trend"subtitle="Last 6 months · inspections count vs revenue ($K)"action={
          ...
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%"height="100%">
              <LineChart data={inspectionsByMonth} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                ...
                <Line
                  yAxisId="left"type="monotone"dataKey="inspections"stroke="#6366f1"strokeWidth={2.5}
                  dot={{ r: 4, fill:"#6366f1"}}
                  activeDot={{ r: 6 }}
                  name="Inspections"/>
                <Line
                  yAxisId="right"type="monotone"dataKey="revenue"stroke="#10b981"strokeWidth={2.5}
                  strokeDasharray="5 5"dot={{ r: 4, fill:"#10b981"}}
                  name="Revenue ($K)"/>
              </LineChart>
            </ResponsiveContainer>
          </div>
            ...
            <ResponsiveContainer width="100%"height="100%">
              <PieChart>
                <Pie
                  ...
                  dataKey="value"nameKey="name"innerRadius={55}
                  outerRadius={85}
                  paddingAngle={2}
                >
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader
            title="Inspector Performance"subtitle="Completed inspections and NCRs issued per inspector"action={<Button variant="outline"size="sm">Export <ArrowUpRight className="h-3.5 w-3.5"/></Button>}
          />
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%"height="100%">
            ...
            {recentActivity.map((a, i) => (
              <div key={i} className="flex items-start gap-3 px-5 py-3">
                <Avatar name={a.who} size="sm"gradient={
                  ...
                  a.tone ==="amber"?"from-amber-500 to-orange-600":"from-indigo-500 to-violet-600"} />
      ...
      <Card>
        <CardHeader
          title="Inspection Pipeline"subtitle="Live snapshot of the 5-step workflow"action={<Button size="sm">View Workflow <ArrowRight className="h-3.5 w-3.5"/></Button>}
        />
