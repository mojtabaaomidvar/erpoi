            ...
            {inspectionsByMonth.map((d, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-2 justify-end h-full">
                <div className="w-full flex flex-col gap-1 flex-1 justify-end">
                  <div
                    className="w-full bg-gradient-to-t from-indigo-500 to-violet-500 rounded-t-md"style={{ height: `${(d.revenue / 250) * 100}%` }}
                  />
                </div>
            ...
            {inspectionsByDiscipline.map((d) => (
              <div key={d.name} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                  <div
                    className="h-full rounded-full"style={{ width: `${(d.value / 30) * 100}%`, background: d.color }}
                  />
                </div>
            ...
            .map((ins, idx) => (
              <div key={ins.id} className="flex items-center gap-4 px-5 py-3">
                ...
                <div className="h-2 w-32 overflow-hidden rounded-full bg-slate-100">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-indigo-500 to-violet-500"style={{ width: `${(ins.completedJobs / 150) * 100}%` }}
                  />
                </div>
              </div>
            ))}
